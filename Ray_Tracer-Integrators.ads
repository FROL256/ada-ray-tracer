...
private package Ray_Tracer.Integrators is
  ...
  type vector32i is array (0..QMC_KMLT_MAXRANDS) of integer;
  type vector32f is array (0..QMC_KMLT_MAXRANDS) of float;


  type QMC_KMLT_Generator is new RandomGenerator with record
    indices : vector32i := (others => 0);
    values  : vector32f := (others => 0.0);
    top     : integer   := 0;
  end record;

  function rnd_uniform_dispatch(gen : access QMC_KMLT_Generator; l,h : float) return float;

  procedure PrimarySample(self : in QMC_KMLT_Generator; res : out float);
  procedure Push(self : in QMC_KMLT_Generator; i : in integer; val : in float);

  procedure Pop(self : in QMC_KMLT_Generator; i : out integer; val : out float);

  type QMCGenRef is access QMC_KMLT_Generator;
  ...
  function Mutate(self : in MLTKelmenSimple; a_value : float) return float;
